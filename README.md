...
- A usage-plan API key authorizes the whole stage, not one resource. Per-tool keys give you independent revocation, but any valid key can still reach any action endpoint. Each action re-validates device identity and runs under its own least-privilege Jamf client, so a key aimed at another action gains only that action's scoped, identity-checked behavior. To bind a key to one action, check the caller's `apiKeyId` in the Lambda and reject anything outside its allowlist.
